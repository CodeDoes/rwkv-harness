...
export interface TraceFormatOpts {
  ...
  /**
   * Override traces directory. Defaults to `$TRACE_DIR` env var, then
   * `<cwd>/.traces/`. An explicit value is recommended for tests so they
   * can write into a tempdir without polluting the project root.
   */
  tracesDir?: string
}
...
export class TraceWriter {
  ...
  constructor(mode: string, opts: TraceFormatOpts = {}) {
    const dir = opts.tracesDir ?? defaultTracesDir()
    fs.mkdirSync(dir, { recursive: true })
    const ts = new Date().toISOString().replace(/[:.]/g, "-")
    this.filePath = path.join(dir, `${ts}_${mode}.txt`)
