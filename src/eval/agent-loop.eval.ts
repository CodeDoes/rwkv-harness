...
// ── Test 2: EOT (\x03) signals end of turn ──
async function testEotSignalsEnd(baseDir: string): Promise<boolean> {
  console.error("\n── Test: EOT (\\x03) signals end of turn ──")

  const trace = new TraceWriter("eot").open()
  const model = EvalController.createMockModel([
  ...
  const checks: Check[] = [
    ...
    { name: "no extra iterations (EOT breaks loop)", pass: model.callCount === 1 },
  ]

  const allPass = EvalController.reportVerification("EOT Test", checks, trace)
  trace.close()
  return allPass
}
...
async function main(): Promise<number> {
  ...
  const results = await Promise.all([
    ...
    testEotSignalsEnd(baseDir).catch(e => { console.error("EOT test error:", e); return false }),
